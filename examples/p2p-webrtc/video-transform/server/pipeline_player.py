...
class PlayPipelineFrame(Frame):
    """Request to play a GStreamer pipeline."""
...
class PlayPipelineEndFrame(Frame):
    """Indicates that a PlayPipelineFrame has finished."""
...
class GStreamerPipelinePlayer(FrameProcessor):
    ...
    def __init__(self, *, out_params: OutputParams = OutputParams(), **kwargs):
        super().__init__(**kwargs)
        self._out_params = out_params
        self._sample_rate = 0

        self._pipeline_queue: asyncio.Queue[str] = asyncio.Queue()
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, StartFrame):
            ...
            await self.push_frame(frame, direction)
        elif isinstance(frame, PlayPipelineFrame):
    ...
    async def _start(self, frame: StartFrame):
        ...
        if not self._pipeline_task:
            self._pipeline_task = self.create_task(self._pipeline_loop(), name="pipeline-loop")

    async def _stop(self, frame: EndFrame):
        await self._stop_current_pipeline()
    ...
    async def _pipeline_loop(self):
        while True:
            ...
            try:
                ...
                logger.exception(f"{self}: error running pipeline: {e}")
                error = str(e)
    ...
    async def _play_pipeline(self, pipeline_description: str) -> Optional[str]:
        ...
        self._pipeline_future = loop.run_in_executor(
            None, self._run_pipeline_sync, pipeline_description
        )
        return await self._pipeline_future

    def _run_pipeline_sync(self, pipeline_description: str) -> Optional[str]:
        ...
        def decodebin_callback(decodebin: Gst.Element, pad: Gst.Pad):
        ...
        decodebin.connect("pad-added", decodebin_callback)
    ...
    async def _stop_current_pipeline(self):
        if self._glib_loop and self._glib_loop.is_running():
            self._glib_loop.quit()
        if self._pipeline_future:
            try:
                await asyncio.wait_for(self._pipeline_future, timeout=5)
            except Exception:
                pass
